...
var _border_lines: MeshInstance3D
...
func _debug_colour_for_coord(chunk_coord: Vector2i) -> Color:
	...
	return Color.from_hsv(hue, 0.30, 0.70, 1.0)
...
func _update_debug_border() -> void:
	if not is_node_ready() or _border_lines != null:
		return

	var size := ChunkCoordinates.CHUNK_SIZE_M
	var y := 0.08
	var vertices := PackedVector3Array([
		Vector3(0.0, y, 0.0), Vector3(size, y, 0.0),
		Vector3(size, y, 0.0), Vector3(size, y, size),
		Vector3(size, y, size), Vector3(0.0, y, size),
		Vector3(0.0, y, size), Vector3(0.0, y, 0.0),
	])
	var arrays := []
	...
	_border_lines.name = "ChunkBorder"
	_border_lines.mesh = mesh
	_border_lines.material_override = material
	_debug_overlay.add_child(_border_lines)
