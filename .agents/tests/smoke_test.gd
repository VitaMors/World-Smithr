...
func _init() -> void:
	...
	var restored_document := WorldDocumentScript.new()
	restored_document.load_height_samples(world_document.serialize_height_samples())
	_expect_equal(restored_document.get_chunk_data(Vector2i(1, 1)).get_height(0, 0), 9.0, errors, "serialized height restore")
